...
void mainImage( out vec4 fragColor, in vec2 fragCoord ) {
    ...
    vec3 ro = vec3(PI/2., 0.0, -t * 0.5);
    vec3 rd = normalize(vec3(uv, -0.5));

    // Default camera rotation logic
    rd.xy = rot(sin(t * 0.2)) * rd.xy;
    vec3 ta = vec3(cos(t * 0.4), sin(t * 0.4), 4.0);
...
void main() {
    // 1. Prepare the output variable
    vec4 color = vec4(0.0);

    // 2. Call the Shadertoy logic using standard GL inputs
    // gl_FragCoord contains the pixel coordinates (x, y)
    mainImage(color, gl_FragCoord.xy);

    // 3. Assign to the standard output
    gl_FragColor = color;
}